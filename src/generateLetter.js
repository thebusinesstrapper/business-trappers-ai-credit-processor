...
export async function generateLetters(chain, analysis, context = {}) {
    ...
    for (const [bureau, items] of [...byBureau.entries()].sort()) {
        ...
        for (const item of items) {
            ...
            // A tradeline dispute section must identify the furnisher.
            //
            // A blank furnisher is not a cosmetic issue: the bureau may be unable
            // to identify which account is being disputed. Do not substitute a
            // generic label, guess a creditor, or borrow a name from another
            // bureau. Withhold only this item and allow the remaining trustworthy
