...
/**
 * Persist a POSITIVELY CONFIRMED inactive -> active reactivation.
 *
 * Called ONLY from the daily inactive-recheck sweep, and ONLY when the live
 * CreditHero landing was a positively confirmed healthy member dashboard. There
 * is deliberately no stored-state-only path into this writer: a reactivation is
 * a live fact, never inferred from memory alone.
 *
 * IDEMPOTENT. monitoring_reactivated_date is set ONLY on the transition — the
 * update is guarded by `.is("monitoring_reactivated_date", null)`, so a client
 * that is already active keeps its ORIGINAL reactivation date across every later
 * run. The access-state / check-time / block-clear writes are unconditional and
 * safe to repeat.
 *
 * Narrow by design: it flips credit_hero_access_state to active, stamps the
 * check time, records the reactivation date (once), and clears the stale inactive
 * block_reason. It does NOT touch current_round, processing_state, delivery
 * locks, crc_client_status, or any dispute timestamp.
 *
 * @param {string|number} crcClientId
 * @param {string} nowIso  ISO timestamp for the reactivation + check time.
 * @returns {Promise<{ok:boolean, reactivatedDate?:string|null, firstReactivation?:boolean, reason?:string}>}
 */
export async function recordMonitoringReactivated(crcClientId, nowIso = new Date().toISOString()) {
    const id = String(crcClientId);

    if (!/^\d+$/.test(id)) {
        throw new Error(`recordMonitoringReactivated: invalid crcClientId "${crcClientId}".`);
    }
    ...
    if (stateErr) {
        throw new Error(`Failed to reconcile reactivated client state: ${stateErr.message}`);
    }

    if (!stateRow) return { ok: false, reason: "client_state_row_not_found" };
...
/**
 * Enumerate clients whose STORED state is Credit Monitoring Inactive, for the
 * daily read-only recheck sweep. Supabase is the primary enumeration source; the
 * caller merges these with CRC-grid rows whose visible status is inactive and
 * deduplicates by crc_client_id.
 *
 * Narrow by design: fixed filter, fixed field set. Not a generic repository
 * query. Completed clients are excluded (they are terminal).
 *
 * @returns {Promise<Array<object>>}  rows with the identify-and-recheck fields.
 */
export async function listInactiveClients() {
    ...
    if (error) {
        throw new Error(`Failed to list inactive clients: ${error.message}`);
    }
