...
export function buildM7Diagnostic(m7, clientName = null) {
    ...
    return {
        ...
        // Stage-2 (Confirm Payment Information -> thank-you) diagnostics. SAFE
        // ONLY: page/price/flow signals, never any card/token/billing/payload.
        reachedConfirmPaymentPage: safeBool(capture?.reachedConfirmPaymentPage),
        confirmProductMatched: safeBool(capture?.confirmProductMatched),
        confirmedPrice: safeCode(capture?.confirmedPrice) ?? null,
        secondSubmitPresent: safeBool(capture?.secondSubmitPresent),
        secondSubmitClicked: safeBool(capture?.secondSubmitClicked),
        orderPostObserved: safeBool(capture?.orderPostObserved),
        orderPostHttpStatus: safeNumber(capture?.orderPostHttpStatus),
        reachedThankYouPage: safeBool(capture?.reachedThankYouPage),
        newerReportDetected: safeBool(capture?.newerReportDetected),
        reportDateBefore: safeIsoDate(capture?.reportDateBefore) ?? null,
        reportDateAfter: safeIsoDate(capture?.reportDateAfter) ?? null,
        viewReportClicked: safeBool(capture?.viewReportClicked),
